...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        cleaner=DataCleaner()
        df=make_sample_df()
        results=cleaner.drop_invalid_rows(df,["name","age"])

        self.assertEqual(results.isna().sum().sum(),0)
        self.assertLess(len(results),len(df))
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        results=cleaner.trim_strings(df,["name"])

        self.assertEqual(df["name"][0]," Alice ")

        self.assertEqual(results["name"][0],"Alice")
        self.assertEqual(results["name"][3],"Carol")

        pdt.assert_series_equal(results["city"],df["city"])
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        results=cleaner.remove_outliers_iqr(df,"age",factor=0.5)

        self.assertNotIn(120,results["age"].values)
        self.assertIn(35,results["age"].values)
