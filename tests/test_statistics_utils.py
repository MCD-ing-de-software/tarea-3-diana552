...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        utils=StatisticsUtils()
        arr=[1,2,3,4]
        results=utils.moving_average(arr, window=2)
        expected=np.array([1.5,2.5,3.5])

        npt.assert_allclose(results,expected,rtol=1e-10, atol=1e-10)
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        utils=StatisticsUtils()
        arr=[10,20,30,40]
        results=utils.zscore(arr)
        self.assertAlmostEqual(results.mean(),0)
        self.assertAlmostEqual(results.std(),1)
